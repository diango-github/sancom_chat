...
const g_elementCheckboxDisplay = document.getElementById( "checkbox_display" );
...
const g_aiLettering= document.getElementById( "ai_lettering");
const g_translating_en= document.getElementById( "ai_translating_en");
const g_translating_ch= document.getElementById( "ai_translating_ch");
const g_translating_ko= document.getElementById( "ai_translating_ko");

//Web Speech API
const g_speech = new webkitSpeechRecognition();
g_speech.lang = 'ja-JP';
g_speech.start();
...
function onclickCheckbox_CameraMicrophone()
{
    ...
    let bDisplay_new = false;
    if( g_elementCheckboxDisplay.checked )
    {
        bDisplay_new = true;
    }
    ...
    if( bCamera_old === bCamera_new && bMicrophone_old === bMicrophone_new && !bDisplay_new )
    {   // チェックボックスの状態の変化なし
        return;
    }
    ...
    if( !bCamera_new && !bMicrophone_new && !bDisplay_new )
    ...
    if( bDisplay_new )
    {
        async function myAsync() {
            //let stream = null;
            try {
                console.log( "Call : navigator.mediaDevices.getUserMedia( video=%s, audio=%s )", bDisplay_new, bMicrophone_new );
                const streamVideo = await navigator.mediaDevices.getDisplayMedia( { video: bDisplay_new } );
                const streamAudio = await navigator.mediaDevices.getUserMedia( {video:bCamera_new, audio:bMicrophone_new} );
                const [displayVideoTrack] = streamVideo.getVideoTracks();
                const [userAudioTrack] = streamAudio.getAudioTracks();
                const stream = new MediaStream( [displayVideoTrack, userAudioTrack] );
                ...
                console.log( "Call : setStreamToElement( Video_Local, stre  am )" );
                ...
                g_elementCheckboxDisplay.checked = false;
                return;
            }
        }
        myAsync();
...
function onsubmitButton_SendMessage()
{
    ...
    g_mapRtcPeerConnection.forEach( ( rtcPeerConnection ) =>
    {
        g_elementTextMessageForSend.value = g_elementTextUserName.value + ":" + g_elementTextMessageForSend.value;
...
g_speech.onresult = function (e) {
    g_speech.stop();
    if(e.results[0].isFinal) {
        let lettering = g_elementTextUserName.value + ":" + e.results[0][0].transcript;
        g_mapRtcPeerConnection.forEach( ( rtcPeerConnection ) =>
        {
            console.log( "- Send Message through DataChannel" );
            rtcPeerConnection.datachannel.send( JSON.stringify( { type: "message1", data: lettering } ) );
        } );    
        // 送信メッセージをメッセージテキストエリアへ追加
        g_aiLettering.value = lettering + "\n" + g_aiLettering.value; // 一番上に追加

        //g_socket.emit("signaling", {type: "translating", data:e.results[0][0].transcript});
        g_socket.emit("signaling", {type: "translating", data: lettering});        
...
const startRecordButton = document.getElementById("start_record_button");
const stopRecordButton = document.getElementById("stop_record_button");
...
startRecordButton.addEventListener("click", () => {
    ...
    g_recorder.ondataavailable = function(evt) {
        ...
        anchor.href = blobUrl;  
        anchor.textContent = "ここからダウンロード"
        g_mapRtcPeerConnection.forEach( ( rtcPeerConnection ) =>
        {
            console.log( "- Send arrayBuffer through DataChannel" );
            rtcPeerConnection.datachannel.binaryType = 'arraybuffer';
            videoBlob.arrayBuffer().then( arrayBuffer => {
                const MAXIMUM_MESSAGE_SIZE = 65535;
                const END_OF_FILE_MESSAGE = 'EOF';
                for (let i = 0; i < arrayBuffer.byteLength; i += MAXIMUM_MESSAGE_SIZE) {
                    rtcPeerConnection.datachannel.send(arrayBuffer.slice(i, i + MAXIMUM_MESSAGE_SIZE));
                  }
                rtcPeerConnection.datachannel.send( END_OF_FILE_MESSAGE );
            } );
        } );
        //playbackVideo.play();
    }
    g_recorder.start();
    anchor.textContent = "収録中"    
...
g_socket.on(
    ...
    ( objData ) =>
    {
        ...
        else if( "translating" === objData.type )
        {
             g_translating_en.value = objData.data[0] + "\n" + g_translating_en.value; // 一番上に追加
             g_translating_ch.value = objData.data[1] + "\n" + g_translating_ch.value;
             g_translating_ko.value = objData.data[2] + "\n" + g_translating_ko.value;
        }
        else
        {
...
function setupDataChannelEventHandler( rtcPeerConnection )
{
    ...
    {
      if(typeof event.data === 'object' || event.data === 'EOF')
      {
        rtcPeerConnection.datachannel.binaryType = 'arraybuffer'; 
        const END_OF_FILE_MESSAGE = 'EOF';
        if (event.data !== END_OF_FILE_MESSAGE) {
            g_receivedBuffers.push(event.data);
        }
        else {
            console.log( "receivedBuffers[] :", g_receivedBuffers );
            const arrayBuffer = g_receivedBuffers.reduce((acc, arrayBuffer) => {
            ...
            var elementUrlRemote = getRemoteUrlElement(rtcPeerConnection.strRemoteSocketID);
            elementUrlRemote.download = 'recorded.webm';
            elementUrlRemote.href = blobUrl; //objData.data; 
            elementUrlRemote.textContent = "　共有ファイル(ダウンロード)";  
            g_receivedBuffers = [];     
        }
      }
      else
      {
        ...
        else if( "message1" === objData.type )
        {
            // 受信メッセージをメッセージテキストエリアへ追加
            //let strMessage = objData.data;
            g_aiLettering.value = objData.data + "\n" + g_aiLettering.value; // 一番上に追加
            //g_elementTextareaMessageReceived.value += strMessage + "\n";  // 一番下に追加
        }
        else if( "offer" === objData.type )
        {
...
function appendRemoteInfoElement( strRemoteSocketID, strUserName )
{
    ...
    let strElementUrlID = "url_"+ strRemoteSocketID;
    ...
    elementVideo.setAttribute('playsinline', '');
    ...
    elementUrl.id = strElementUrlID;
    elementUrl.class= "download";
    elementUrl.href = "#";
    ...
    elementDiv.appendChild( elementUrl );     //Url
...
//リモートUrl用のHTML要素の取得
function getRemoteUrlElement( strRemoteSocketID )
{
    let strElementUrlID = "url_" + strRemoteSocketID;

    return document.getElementById( strElementUrlID );
}
